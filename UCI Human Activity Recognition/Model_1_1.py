...
from keras.layers import Conv1D, Dropout, MaxPooling1D, Flatten, Dense, concatenate
...
def fit_multi_channel_model(train_x, train_y, filter_size, kernel_size, verbose=0, epochs=10, batch_size=32):
    n_time_steps, n_features, n_outputs = train_x.shape[1], train_x.shape[2], train_y.shape[1]
    # head 1
    inputs1 = Input(shape=(n_time_steps, n_features))
    conv_1 = Conv1D(filters=filter_size, kernel_size=kernel_size, activation='relu')(inputs1)
    drop1 = Dropout(0.5)(conv_1)
    pool1 = MaxPooling1D(pool_size=2)(drop1)
    flat1 = Flatten()(pool1)

    # head 2
    inputs2 = Input(shape=(n_time_steps, n_features))
    conv_2 = Conv1D(filters=filter_size, kernel_size=kernel_size, activation='relu')(inputs2)
    drop2 = Dropout(0.5)(conv_2)
    pool2 = MaxPooling1D(pool_size=2)(drop2)
    flat2 = Flatten()(pool2)

    # head 3
    inputs3 = Input(shape=(n_time_steps, n_features))
    conv_3 = Conv1D(filters=filter_size, kernel_size=kernel_size, activation='relu')(inputs3)
    drop3 = Dropout(0.5)(conv_3)
...
def evaluate_multi_channel_model(model, test_x, test_y, batch_size):
    # evaluate model
...
# run an experiment
def run_experiment(multi_channel, standardize_list, n_filters, n_kernels, repeats=10):
    # load data
        ...
        for filter_size in n_filters:
            ...
            for kernel_size in n_kernels:
                ...
                for r in range(repeats):
                    ...
                    score = 0
                    if multi_channel:
                        model = fit_multi_channel_model(train_x, train_y, filter_size, kernel_size, verbose, epochs,
                                                        batch_size)
                        score = evaluate_multi_channel_model(model, test_x, test_y, batch_size)
                    else:
